<template>
    <view v-if="showTag" :class="showTagBox? 'tag-section-box':'section-box'">
        ...
        <u-tag v-show="!showTagBox" :text="chosenTag" bg-color="#f4756b" border-color="#f4756b" color="#f7fafd"
               shape="circle" :closeable="true" @close="closeTag"/>
        <scroll-view v-show="showTagBox" scroll-x="true" class="tap-box-list">
            <view class="tag-box" v-for="(item, index) in tagArray" :key="index">
                <u-tag :text="item" bg-color="#f4756b" border-color="#f4756b" color="#f7fafd" shape="circle"
...
.tag-section-box {
    ...
    border-bottom: 1 rpx solid #f0f1f5;
