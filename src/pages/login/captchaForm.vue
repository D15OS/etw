<template>
    <!-- 注册表单 -->
    <view class="captcha-form" :class="captchaFormAnimation" :style="{display: showCaptchaForm}">
        <text class="title" v-if="captchaUsernameType === 1">电子邮箱验证</text>
        <text class="title" v-else>手机号验证</text>
        <!-- S 输入框区域 -->
        <view class="input-area">
            <input class="captcha-raw-input" type="number" maxlength="6" @input="captchaInputWatcher" adjust-position="false"
                   v-model="captchaRawInputValue" :focus="captchaRawInputFocusState" @blur="captchaInputBlur">
            <view class="captcha-info">
                <text>验证码已发送至 {{ captchaUsername }}</text>
            </view>
            <view class="captcha-show-area">
                <view v-for="item in 6" :key="item" class="show-content"
                      :style="{borderBottomColor: captchaInputShowColor[item]}" @click="captchaInputFocus">
        ...
        <view class="resend-captcha">
            <text v-if="captchaResendDelay === 0" @click="resendCaptcha">重新发送</text>
